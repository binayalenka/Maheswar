...
/*-----------------CHANGE USERNAME---------------------------------------*/
	...
	function admin_game_admins()
	{
		$this->layout="admin";
		$page= isset($this->params['named']['page'])?$this->params['named']['page']:'1';
		
		$counter = $this->User->find('count',array('conditions'=>array('User.id'=>'1')));
		
		$this->paginate=array('limit'=>$this->pageLimit,'order'=>'User.id DESC');
		$info = $this->paginate('User',array('User.id'=>'1'));
	...
	function admin_game_users()
	{
		$this->layout='admin';		
		$page=isset($this->params['named']['page'])?$this->params['named']['page']:'1';
		//pr($this->params); die;
		$counter = $this->User->find('count',array('conditions'=>array('User.id'=>'2')));
		
		$this->paginate=array('limit'=>$this->pageLimit,'order'=>'User.id DESC');
		$info=$this->paginate('User',array('User.id'=>'2'));
		$this->set(compact('info','page','counter'));
...
//-----------------------------------------------------------Instructor Add Profile-----------------------------------
	function admin_add_game_user()
	{
		...
		//---------------------------------------------Load Model------------------------------
			$this->loadModel('User');
		//---------------------------------------------End Load Model------------------------------
		$lastUserDetail=$this->User->find('first',array('order'=>'User.id desc'));
		$error = array();
		if(!empty($this->data))
		{
			$error = $this->validate_add_profile_steps($this->data);					
			$data = $this->data;
			if(isset($lastUserDetail['User']['unique_id']) || empty($lastUserDetail))
			{
				if(empty($lastUserDetail)){
					$data['User']['unique_id'] = 1000;
				}elseif($lastUserDetail['User']['unique_id']==''){
					$data['User']['unique_id'] = 1000;
				}else{
					$data['User']['unique_id'] = $lastUserDetail['User']['unique_id'] +1;
				}
			}
			...
			if(count($error) == "0"){
				...
				//$data['User']['password'] = Security::cipher($this->data['User']['fpassword'],'2');
				$saved = $this->User->save($data);
				$lastMemId = $this->User->getLastInsertId($data);
				...
				$memDetails = $this->User->findById($lastMemId);
				
				if($saved){
					$emailTemplate=$this->EmailTemplate->find('first',array('conditions'=>array('EmailTemplate.id'=>7)));
					$emailData=$emailTemplate['EmailTemplate']['description'];						
					$emailData=str_replace(array('{name}','{last_name}','{email}','{password}'),array($memDetails['User']['first_name'],$memDetails['User']['last_name'],$memDetails['User']['email'],$this->data['User']['fpassword']),$emailData);							
					$emailTo=$memDetails['User']['email'];
					$emailFrom=$emailTemplate['EmailTemplate']['email_from'];
					$emailSubject=$emailTemplate['EmailTemplate']['subject'];
					$this->send_email($emailTo,$emailFrom,$emailSubject,$emailData);
	...
	{			
		...
		elseif(trim($data['User']['email'])!="")
		{
			...
			$checkexistEmail=$this->User->find('count',array('conditions'=>array('email'=>$data['User']['email'])));
...
//-----------------------------------------------------------Instructor Edit Profile----------------------------------
	function admin_edit_game_admin($m_id=NULL)
	{
		$this->layout="admin";
		...
		$login_member_details=$this->User->find('first',array('conditions'=>array('User.id'=>$id),'recursive'=>2));
		...
		if(isset($login_member_details['UserCategory'][0]['cate_id']))
		{	
			if(!empty($login_member_details['UserCategory'][0]['cate_id']))
			{	
				$sub_category_id = $this->UserCategory->find('all',array('conditions'=>array('UserCategory.cate_id'=>$login_member_details['UserCategory'][0]['cate_id'],'UserCategory.m_id'=>$id)));
				
				$show_sub_cate = array();
				foreach($sub_category_id as $sub_category_id){ 
					
					$show_sub_cate[] = $sub_category_id['UserCategory']['sub_cate_id'];
					
				}
				$this->set(compact('show_sub_cate'));
				
				$sub_category_list=$this->SubCategory->find('all',array('conditions'=>array('SubCategory.status'=>1,'SubCategory.c_id'=>$login_member_details['UserCategory'][0]['cate_id'])));
				$sub_category_list=Set::combine($sub_category_list,'{n}.SubCategory.id','{n}.SubCategory.name');
			}
		}
		if(!empty($login_member_details['User']['cate_id'])){
			$sub_category=$this->SubCategory->find('all',array('conditions'=>array('SubCategory.status'=>1,'SubCategory.c_id'=>$login_member_details['User']['cate_id'])));
		...
		if(!empty($this->data))
		{
			...
			if(count($errors) == "0"){
				...
				$login_member_details = $this->User->findById($data['User']['id']);
				
				if(isset($data['User']['image'])){
					if($data['User']['image']['name']!=''){
						$imgName = pathinfo($data['User']['image']['name']);			
						$ext = strtolower($imgName['extension']);					
						if($ext=='jpg' || $ext=='jpeg' || $ext=='png' || $ext=='gif'){					
							$destination = realpath('../../app/webroot/files').'/'.$data['User']['id'].'/';
							$filename = time().'-'.$data['User']['image']['name'];
							$old_image = $login_member_details['User']['image'];
							$file = $this->data['User']['image'];
							$result1 = $this->Upload->upload($file, $destination, $filename, array());
							$data['User']['image']=$filename;						
							if($result1 && !empty($old_image) && file_exists('files/'.$data['User']['id'].'/'.$old_image)){					
								unlink('../files/'.$data['User']['id'].'/'.$old_image);
							}
						}					
					}
					else
					{
						$data['User']['image']=$login_member_details['User']['image'];
					}	
				}
				$this->UserCategory->deleteAll(array('UserCategory.m_id'=>$data['User']['id']));
				foreach($data['User']['sub_cate_id'] as $subCate)
				{
					$subCategory['UserCategory']['m_id'] = $data['User']['id'];
					$subCategory['UserCategory']['cate_id'] = $data['User']['cate_id'];
					$subCategory['UserCategory']['sub_cate_id'] =$subCate;
					$this->UserCategory->create();
					$this->UserCategory->save($subCategory);
				}
			
				if($data['User']['other_sub_cat']==1)
				{	
					$data['User']['cate_id'] = $data['User']['cate_id'];
					$data['User']['sub_category_name'] = $data['User']['sub_category_name'];
				}elseif($data['User']['other_sub_cat']==0){
					$data['User']['sub_category_name'] = "";
				}
	...
	{			
		...
		if(isset($data['User']['cate_id'])){
			if(trim($data['User']['cate_id'])=="")	{
		...
		if(isset($data['User']['image'])) { 
			if(is_array($data['User']['image'])){ 
								 
				if($data['User']['image']['name']!='')
				{ 	
					$fileName = pathinfo($data['User']['image']['name']);			
		...
		if(empty($data['User']['sub_category_name']) && !isset($data['User']['sub_category_name'])){
			if(isset($data['User']['sub_cate_id'])){
				if(isset($data['User']['sub_cate_id'][0])){
					if($data['User']['sub_cate_id'][0]=="")	{
		...
		if($data['User']['other_sub_cat']=='1')
		{
			if(empty($data['User']['sub_category_name']))
			{
		...
		if(isset($data['User']['city'])){
			if(trim($data['User']['city'])=="")	{
		...
		if($data['User']['phone']!=''){
			if($data['User']['phone']!='' && !ctype_digit($data['User']['phone'])){
				$errors	['phone'][] = __(NUMERIC_VALUE_ONLY,true)."\n";
			}
			elseif($data['User']['phone']!='' && strlen($data['User']['phone'])!='10'){
				$errors	['phone'][] = __(TEN_DIGIT_LIMIT,true)."\n";
			}
		}
		
		if(isset($data['User']['hourly_rate'])){
			
			if($data['User']['hourly_rate']!='' && !ctype_digit($data['User']['hourly_rate'])){
				$errors	['hourly_rate'][] = __(NUMERIC_VALUE_ONLY,true)."\n";
			}
		}
		if(isset($data['User']['experience'])){
			if(trim($data['User']['experience'])=="")	{
		...
		if(isset($data['User']['objective'])){
			if(trim($data['User']['objective'])=="")	{
		...
		if(isset($data['User']['primary_services'])){
			if(trim($data['User']['primary_services'])=="")	{
		...
		if(isset($data['User']['additional_information'])){
			if(trim($data['User']['additional_information'])=="")	{
	...
	function admin_view_game_admin($id=NULL)
	{
		$this->layout="admin";
		...
		$info = $this->User->find('first',array('conditions'=>array('User.id'=>$id),'contain'=>array('UserCategory'=>array('SubCategory.name'),'Country','VideoLink','Category.name')));
		//pr($info);die;
		if(file_exists('files/'. $info['User']['id'])){
			App::uses('Folder','Utility');
			$dir = new Folder('files/'. $info['User']['id']);
			...
			$memory_limit_for_user = $info['User']['member_type'] = 1 ? 5 : 2;
	...
	function admin_view_game_user($id=NULL)
	{
		$this->layout="admin";
		$id = convert_uudecode(base64_decode($id));
	...
	function admin_edit_game_user($id=NULL)
	{
		$this->layout = "admin";
		...
		//---------------------------------------------End Load Model------------------------------
		$id = convert_uudecode(base64_decode($id));
		$login_member_details = $this->User->find('first',array('conditions'=>array('User.id'=>$id)));
		//pr($login_member_details); die;	
	...
	{			
		...
		if(isset($data['User']['street'])){
			if(trim($data['User']['street'])=="")	{
		...
		if(isset($data['User']['phone'])){
			if(trim($data['User']['phone'])=="")	{
				$errors ['phone'] [] = __(FIELD_REQUIRED,true)."\n";
			}
			elseif($data['User']['phone']!='' && !ctype_digit($data['User']['phone'])){
				$errors	['phone'][] = __(NUMERIC_VALUE_ONLY,true)."\n";
			}
			elseif($data['User']['phone']!='' && strlen($data['User']['phone'])!='10'){
				$errors	['phone'][] = __(TEN_DIGIT_LIMIT,true)."\n";
	...
	function admin_update_member_status($page=NULL,$id=NULL,$tableName=NULL,$renderPath=NULL,$renderElement=NULL)
	{
		...
		if($this->RequestHandler->isAjax()){		
			...
			$this->paginate=array('limit'=>$this->pageLimit,'page'=>$page,'order'=>$tableName.'.id DESC','conditions'=>array($tableName.'.member_type'=>$getRec['User']['member_type']));
			$info = $this->paginate($tableName);
			$this->set(compact('info','page'));			
			$this->viewPath = 'Elements'.DS.'adminElements/'.$renderPath;
			$this->render($renderElement);			
	...
	function admin_delete_member_record($page=NULL,$d_id=NULL,$tableName=NULL,$renderPath=NULL,$renderElement=NULL,$mem=NULL)
	{
		...
		{		
			...
			if(!empty($getRec))
			{
				...
				$conditions=array('User.id'=>$member_type);
				if($member_type== '1'){
	...
	function admin_instructorSearch($condition=null)
	{
		...
		{
			if($this->data['User']['type']=="first_name" && trim($this->data['User']['text'])!=''){
				$condition=array_merge($condition,array("User.first_name LIKE"=>"%".trim($this->data['User']['text'])."%"));
				$qryStr=$qryStr."User.first_name LIKE-||".'%'.trim($this->data['User']['text']).'%'."||-";
			}
			if($this->data['User']['type']=="last_name" && trim($this->data['User']['text'])!=''){
				$condition=array_merge($condition,array("User.last_name LIKE"=>"%".trim($this->data['User']['text'])."%"));
				$qryStr=$qryStr."User.last_name LIKE-||".'%'.trim($this->data['User']['text']).'%'."||-";
			}
			if($this->data['User']['type']=="email" && trim($this->data['User']['text'])!=''){
				$condition=array_merge($condition,array("User.email"=>trim($this->data['User']['text'])));
				$qryStr=$qryStr."User.email-||".trim($this->data['User']['text'])."||-";
			}
			if($this->data['User']['type']=="phone" && trim($this->data['User']['text'])!=''){
				$condition=array_merge($condition,array("User.phone"=>trim($this->data['User']['text'])));
				$qryStr=$qryStr."User.phone-||".trim($this->data['User']['text'])."||-";
			}
			if($this->data['User']['type']=="zipcode" && trim($this->data['User']['text'])!=''){
				$condition=array_merge($condition,array("User.zipcode"=>trim($this->data['User']['text'])));
				$qryStr=$qryStr."User.zipcode-||".trim($this->data['User']['text'])."||-";
			}
			$query=$this->User->find('all',array('conditions'=>$condition));
			if(!isset($this->params['named']['condition'])){			
		...
		if($this->RequestHandler->isAjax()) 
		{
			$this->layout="";
			$this->autoRender=false;
			$this->paginate=array('limit'=>$this->pageLimit,'order'=>'User.id DESC','conditions'=>$condition);
			$info = $this->paginate('User',array('User.id'=>'1'));
			$this->set(compact('info','page'));				
			$this->viewPath = 'Elements'.DS.'adminElements/teacher';
	...
	function sort_condition()
	{
		...
		if(isset($this->params['named']['condition']))
		{
			...
			foreach($con as $val):
				$qry=explode("-||",$val);
				//print_r($qry);
				if($qry[0]=="User.first_name LIKE")
				{	
					$var=str_replace("%","",$qry[1]);
					$conditions=array_merge($conditions,array('User.first_name LIKE'=>$qry[1]));
				}
				if($qry[0]=="User.last_name LIKE")
				{
					$var=str_replace("%","",$qry[1]);
					$conditions=array_merge($conditions,array('User.last_name LIKE'=>$var));
				}
				if($qry[0]=="User.email")
				{
					$var=str_replace("%","",$qry[1]);
					$conditions=array_merge($conditions,array('User.email'=>$var));
				}
				if($qry[0]=="User.phone")
				{
					$var=str_replace("%","",$qry[1]);
					$conditions=array_merge($conditions,array('User.phone'=>$var));
				}
				if($qry[0]=="User.zipcode")
				{
					$var=str_replace("%","",$qry[1]);
					$conditions=array_merge($conditions,array('User.zipcode'=>$var));
				}
			endforeach;
		}
		return $conditions;
	...
	function remove_photo_admin($id=NULL)
	{
		...
		if(!empty($record)){
			$data['User']['id'] = $record['User']['id'];
			$data['User']['image'] = "";
			$this->User->save($data);
			
			if($record['User']['image']!='' && file_exists('files'.'/'.$data['User']['id'].'/'.$record['User']['image'])){
				unlink('files'.'/'.$data['User']['id'].'/'.$record['User']['image']);
			}
	...
	function admin_instructor_search_result($query=NULL)
	{
		...
		$condition=array_merge($condition,array('User.id'=>'1')); 
		$info = $this->User->find('all',array('conditions'=>$condition));
		$i=1;
		foreach($info as $info){
			$data .= $i.",";
			$data .= $info['User']['first_name'].' '.$info['User']['last_name'].",";
			$data .= $info['User']['email'].",";
			$data .= $info['User']['phone'].",";
			$data .= $info['User']['city'].",";
			$data .= $info['User']['zipcode'].",";
			$data .= $info['User']['state'].",";
	...
	function admin_student_search_result($query=NULL)
	{
		...
		$condition=array_merge($condition,array('User.id'=>'2')); 
		$info = $this->User->find('all',array('conditions'=>$condition));
		$i=1;
		foreach($info as $info){
			$data .= $i.",";
	...
	function admin_instructor_events()
	{
		...
		$info = $this->User->find('all',array('conditions'=>array('User.id'=>'1')));
		$this->set(compact('info'));
		
		$this->paginate=array('limit'=>$this->pageLimit,'order'=>'User.id DESC');
		$conditions=array('User.id'=>'1');
		$info=$this->paginate('User',$conditions);
		$this->set(compact('info','page'));
		if($this->RequestHandler->isAjax()){
			$this->layout = '';
			$this->autoRender = false;
	...
	function admin_events($id=NULL)
	{
		...
		$login_member_details=$this->User->find('first',array('conditions'=>array('User.id'=>$m_id),'recursive'=>2));
		
		$sub_cate_info = $this->UserCategory->find('list',array('conditions'=>array('UserCategory.m_id'=>$m_id),'fields'=>array('UserCategory.sub_cate_id')));
	...
	function admin_add_event($id=NULL)
	{
		...
		$memberInfo = $this->User->find('first',array('conditions'=>array('User.id'=>$memberId),'recursive'=>2));
		
		$subCatgID = array();
		if(!empty($memberInfo['UserCategory']))
		{
			foreach($memberInfo['UserCategory'] as $memberData)
			{
				$subCatgId[] = $memberData['sub_cate_id'];
			}
			$catgInfo = $this->Category->find('first',array('conditions'=>array('Category.id'=>@$memberInfo['UserCategory'][0]['cate_id'])));
			$sub_category_list = $this->SubCategory->find('list',array('conditions'=>array('SubCategory.id'=>$subCatgId),'fields'=>array('id','name')));
		}
		elseif(!empty($memberInfo['User']['cate_id']) && empty($memberInfo['UserCategory']))
		{
			$sub_category_list = array();
			$catgInfo = $this->Category->find('first',array('conditions'=>array('Category.id'=>$memberInfo['User']['cate_id'])));
		}
		elseif(!empty($memberInfo['User']['cate_id']))
		{
			$catgInfo = $this->Category->find('first',array('conditions'=>array('Category.id'=>$memberInfo['User']['cate_id'])));
		}
	...
	function admin_edit_event($id=NULL,$m_id=NULL)
	{
		...
		$country_list = $this->Country->find('all');
		$country_list = Set::combine($country_list,'{n}.Country.id','{n}.Country.country_name');
		$memberInfo = $this->User->find('first',array('conditions'=>array('User.id'=>$memberId),'recursive'=>'-1'));
		$eventData = $this->Event->find('first',array('conditions'=>array('Event.id'=>$eventId)));
		$memberSubCatg = $this->UserCategory->find('list',array('conditions'=>array('UserCategory.m_id'=>$memberId),'fields'=>array('id','sub_cate_id')));
	...
	function admin_student_events()
	{
		...
		$info = $this->User->find('all',array('conditions'=>array('User.id'=>'2')));
		$this->set(compact('info'));
	...
	function admin_student_view_event($id=NULL,$m_id=NULL)
	{
		...
		$memInfo=$this->User->find('first',array('conditions'=>array('User.id'=>$m_id),'fields'=>array('User.id','User.first_name','User.last_name')));
		$info = $this->Event->find('first',array('conditions'=>array('Event.id'=>$id)));
	...
	function admin_EventSearchStudent()
	{
		...
		if($this->RequestHandler->isAjax()) 
		{
			...
			$this->Event->unBind(array('belongsTo'=>array('Category','SubCategory','User','Country')));
	...
	function sort_event_student_search()
	{
		...
		if(isset($this->params['named']['condition']))
		{
			...
			foreach($con as $val):
				...
				{
					$var=str_replace("%","",$qry[1]);
					$conditions=array_merge($conditions,array('User.class LIKE'=>$var));
				}
			endforeach;
		}
		return $conditions;
	...
	function admin_contactAdminSearch($condition=null)
	{
		...
		else
		{
			if($this->data['ContactAdmin']['type']=="first_name" && trim($this->data['ContactAdmin']['text'])!=''){
				$condition=array_merge($condition,array("User.first_name LIKE"=>"%".trim($this->data['ContactAdmin']['text'])."%"));
				$qryStr=$qryStr."User.first_name LIKE-||".'%'.trim($this->data['ContactAdmin']['text']).'%'."||-";
			}
			if($this->data['ContactAdmin']['type']=="last_name" && trim($this->data['ContactAdmin']['text'])!=''){
				$condition=array_merge($condition,array("User.last_name LIKE"=>"%".trim($this->data['ContactAdmin']['text'])."%"));
				$qryStr=$qryStr."User.last_name LIKE-||".'%'.trim($this->data['ContactAdmin']['text']).'%'."||-";
			}
	...
	function admin_update_status_mail($m_id=NULL)
	{
		...
		if(!empty($this->data))
		{
			...
			if(count($errors)==0){
				...
				$mem_id = base64_encode(convert_uuencode($getRec['User']['id']));
	...
	function admin_add_broadcast($id=NULL)
	{
		...
		if(count($all_events) !=''){
			$this->loadModel('User');
			$this->User->Behaviors->attach('Containable');
			$all_student = $this->User->find('all',array('conditions'=>array('User.id'=>$all_events),'contain'=>array(),'fields'=>array('User.id','User.first_name','User.last_name','User.email')));
		}
					...
					if($this->Broadcast->save($this->data)){
						...
						foreach($s_id as $students){
							...
							if($this->BroadcastStudent->save($data)){
								$student_info = $this->User->find('first',array('conditions'=>array('User.id'=>$students),'fields'=>array('User.email','User.first_name','User.last_name')));	
							
								$emailTemplate = $this->EmailTemplate->find('first',array('conditions'=>array('EmailTemplate.id'=>'31')));
								$emailData = $emailTemplate['EmailTemplate']['description'];						
								$emailData = str_replace(array('{first_name}','{last_name}'),array(ucfirst($student_info['User']['first_name']),ucfirst($student_info['User']['last_name'])),$emailData);							
								   
								$emailTo = $student_info['User']['email'];
								$emailFrom = $emailTemplate['EmailTemplate']['email_from'];
								$emailSubject = $emailTemplate['EmailTemplate']['subject'];
	...
	function admin_view_student_assignment_detail($id=NULL)
	{
		...
		$this->loadModel('InstructorAssignmentDocument');
		$id = convert_uudecode(base64_decode($id));
		$info = $this->InstructorAssignmentStudent->find('first',array('conditions'=>array('InstructorAssignmentStudent.id'=>$id),'contain'=>array('InstructorAssignment','Event'=>array('User'=>array('first_name','last_name')))));
	...
	function admin_assigned_student($id=NULL)
	{
		...
		$id = convert_uudecode(base64_decode($id));
		$this->paginate = array('limit'=>$this->pageLimit,'order'=>'InstructorAssignmentStudent.id DESC','contain'=>array('User'=>array('first_name','last_name'),'InstructorAssignment'=>array('title')));
	...
	function admin_view_sticky_note($id=NULL)
	{
		...
		if(!empty($id))
		{
			...
			$this->paginate = array('order'=>'StickyNote.id DESC','limit'=>'10','fields'=>array('StickyNote.*','User.first_name','User.last_name'));
	...
	function admin_detail_sticky_note($id=NULL)
	{
		...
		{
			$id = convert_uudecode(base64_decode($id));
			$info = $this->StickyNote->find('first',array('conditions'=>array('StickyNote.id'=>$id),'fields'=>array('StickyNote.*','User.first_name','User.last_name')));
			$this->set('info',$info);
		}
	}
	...
	function admin_detail_promotion($id=NULL)
	{
		...
		{
			$id = convert_uudecode(base64_decode($id));
			$info = $this->Promotion->find('first',array('conditions'=>array('Promotion.id'=>$id),'fields'=>array('Promotion.*','User.first_name','User.last_name'),'contain'=>array('User')));
			$this->set('info',$info);
		}
	}
	...
	{ 
		...
		$this->loadModel('SendInvitation');
		$page=isset($this->params['named']['page'])?$this->params['named']['page']:'1';
		$this->paginate=array('limit'=>$this->pageLimit,'order'=>'SendInvitation.id ASC','fields'=>array('SendInvitation.*','Event.title','User.first_name','User.last_name','User.email'));
		$info = $this->paginate('SendInvitation');
	...
	function admin_view_student_invitation($id=NULL)
	{
		...
		if(!empty($id))
		{
			$this->loadModel('SendInvitation');
			$id = convert_uudecode(base64_decode($id));
			$info = $this->SendInvitation->find('first',array('conditions'=>array('SendInvitation.id'=>$id),'fields'=>array('SendInvitation.*','Event.title','User.first_name','User.last_name','User.email')));
			$this->set('info',$info);
		}
	}
